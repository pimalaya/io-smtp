...
//!
//! let domain = SmtpDomain(Cow::Borrowed("example.com"));
...
impl<'a> From<SmtpHeloCommand<'a>> for Vec<u8> {
    fn from(cmd: SmtpHeloCommand<'a>) -> Vec<u8> {
        ...
        buf.push_str(cmd.domain.as_ref());
        buf.push_str("\r\n");
        buf.into_bytes()
    }
}
...
pub enum SmtpHeloError {
    /// The server rejected the HELO command.
...
impl SmtpHelo {
    /// Creates the coroutine from the client domain sent to the
    /// server.
    pub fn new(domain: SmtpDomain<'_>) -> Self {
    ...
    fn resume(&mut self, arg: Option<&[u8]>) -> SmtpCoroutineState<Self::Yield, Self::Return> {
        match &mut self.state {
            State::Send(send) => {
                ...
                SmtpCoroutineState::Complete(Err(SmtpHeloError::Rejected { code, message }))
            }
        }
    }
}
...
enum State {
    Send(SmtpCommandSend<SmtpHeloCommand<'static>>),
}
...
mod tests {
    ...
    fn domain() -> SmtpDomain<'static> {
        SmtpDomain(Cow::Borrowed("example.com"))
    }
    ...
    fn eof_returns_eof_error() {
        ...
        assert!(matches!(
            err,
            SmtpHeloError::Send(SmtpCommandSendError::Eof)
        ));
    }
