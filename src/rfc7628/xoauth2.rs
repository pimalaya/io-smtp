...
impl SmtpCoroutine for SmtpXoauth2 {
    ...
    type Return = Result<(), SmtpXoauth2Error>;
        ...
        loop {
            ...
            match &mut self.state {
                State::AwaitInitial => {
                    match arg.take() {
                        Some(&[]) => {
                            return SmtpCoroutineState::Complete(Err(SmtpXoauth2Error::Eof));
                        }
                        Some(data) => {
                    ...
                    let response = match Response::parse(&self.buf) {
                        ...
                        Err(errors) => {
                            ...
                            return SmtpCoroutineState::Complete(Err(
                                SmtpXoauth2Error::ParseResponse(reason),
                            ));
                        }
                    };
                    ...
                    return SmtpCoroutineState::Complete(Err(SmtpXoauth2Error::Rejected {
                        code,
                        message,
                    }));
                }
                ...
                State::Ehlo(ehlo) => match ehlo.resume(arg.take()) {
                    ...
                    SmtpCoroutineState::Complete(Err(err)) => {
                        return SmtpCoroutineState::Complete(Err(SmtpXoauth2Error::Ehlo(err)));
                    }
